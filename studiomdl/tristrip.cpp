...
void	FindNeighbor (int starttri, int startv)
{
	...
	for (j=starttri+1, check=&triangles[starttri+1][0] ; j<pmesh->numfaces ; j++, check += 3)
	{
		if (used[j] == 7)
			continue;
		for (k=0 ; k<3 ; k++)
...
int BuildTris (s_trianglevert_t (*x)[3], s_mesh_t *y, byte **ppdata )
{
	...
	for (i=0 ; i<pmesh->numfaces ;)
	{
		...
		bestlen = 0;
		m = 0;
		for (k = i; k < pmesh->numfaces && bestlen < 127; k++)
		{
			int localpeak = 0;
		...
		// printf("%d (%d) %d\n", bestlen, pmesh->numfaces - total, i );
		...
		if (t != time(NULL))
		{
			printf("%2d%%\r", (total * 100) / pmesh->numfaces );
			t = time(NULL);
		}
	}
	...
	*ppdata = (byte *)commands;
	...
	// printf("%d %d %d\n", numcommandnodes, numcommands, pmesh->numfaces  );
