...
struct TelemetryData
{
	...
	TelemetryZonePlotData m_ZonePlot[TELEMETRY_ZONE_PLOT_SLOT_MAX];		// Data used to plot a zone's duration when using TM_ZONE_PLOT
};
...
//Description
//	Helper macro in C++ that creates an anonymous local object that automatically calls tmEnter 
//	and then tmLeave when exiting scope. It will also accumulate the time over a frame to plot 
//	the zone duration
//	Note the function are not thread safe!
//
//Parameters
//	cx [in] handle to a valid Telemetry context
//	name [in] name of the zone
//	slot [in] slot use to plot the graph (of type TelemetryZonePlotSlot_t)
