...
int LzmaDecodeWithCallback(CLzmaDecoderState *vs,
#ifdef _LZMA_IN_CB
			   ILzmaInCallback *InCallback,
#else
			   const unsigned char *inStream, SizeT inSize, SizeT *inSizeProcessed,
			   ...
			   unsigned char *outStream, SizeT outSize, SizeT *outSizeProcessed, LZMAReadProgressCallbackFunc_t pCallbackFunc )
{
	...
	*outSizeProcessed = 0;
	...
	while(len != 0 && nowPos < outSize)
	{
		...
		if ( ( nowPos % READ_PROGRESS_CALLBACK_BLOCK_SIZE ) == 0 )
		{
			pCallbackFunc();
		}
			   ...
			   unsigned char *outStream, SizeT outSize, SizeT *outSizeProcessed, LZMAReadProgressCallbackFunc_t pCallbackFunc )
{
	if ( pCallbackFunc )
	{
		// Call a different version that does checks for when to callback so that we don't take a perf hit on this version without.
...
//-----------------------------------------------------------------------------
unsigned int CLZMA::Uncompress( unsigned char *pInput, unsigned char *pOutput, LZMAReadProgressCallbackFunc_t pCallback )
{
	...
	SizeT inProcessed;
	SizeT outProcessed;
	int result = LzmaDecode( &state, pInput + sizeof( lzma_header_t ), lzmaSize, &inProcessed, pOutput, actualSize, &outProcessed, pCallback );
