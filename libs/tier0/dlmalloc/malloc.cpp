...
#pragma warning(disable : 4267) // warning C4267: 'initializing' : conversion from 'size_t' to 'bindex_t', possible loss of data
...
// @TODO: Improve the threading model [5/22/2009 tom]
#define MLOCK_T         CThreadFastMutex
#define CURRENT_THREAD  GetCurrentThreadId()
#define INITIAL_LOCK(s) 
inline int DoThreadLock( CThreadFastMutex *p )
{
	p->Lock();
	return 0;
}
#define ACQUIRE_LOCK(s) DoThreadLock( s )
#define RELEASE_LOCK(s) (s)->Unlock()
#define TRY_LOCK(s)     (s)->TryLock()
#define IS_LOCKED(s)    (s)->IsLocked()
#define NULL_LOCK_INITIALIZER
static MLOCK_T magic_init_mutex;
...
#ifndef ACQUIRE_MAGIC_INIT_LOCK
#if USE_LOCKS
  ...
  */
  ...
  if (MORECORE_CONTIGUOUS && !use_noncontiguous(m)) {
    ...
    if (ss == 0) {  /* First time through or recovery */
      ...
      asize = granularity_align(nb - m->topsize + TOP_FOOT_SIZE + SIZE_T_ONE + ( MALLOC_ALIGNMENT - TWO_SIZE_T_SIZES ) );
    ...
    if (tbase == CMFAIL) {    /* Cope with partial failure */
      if (br != CMFAIL) {    /* Try to use/extend the space we did get */
        if (asize < HALF_MAX_SIZE_T &&
          ...
          size_t esize = granularity_align(nb + TOP_FOOT_SIZE + SIZE_T_ONE  + ( MALLOC_ALIGNMENT - TWO_SIZE_T_SIZES ) - asize);
  ...
  if (HAVE_MMAP && tbase == CMFAIL) {  /* Try MMAP */
    size_t req = nb + TOP_FOOT_SIZE + SIZE_T_ONE + ( MALLOC_ALIGNMENT - TWO_SIZE_T_SIZES );
  ...
  if (HAVE_MORECORE && tbase == CMFAIL) { /* Try noncontiguous MORECORE */
    size_t asize = granularity_align(nb + TOP_FOOT_SIZE + SIZE_T_ONE + ( MALLOC_ALIGNMENT - TWO_SIZE_T_SIZES ) );
    if (asize < HALF_MAX_SIZE_T) {
      char* br = CMFAIL;
      char* end = CMFAIL;
      ACQUIRE_MORECORE_LOCK();
